...
if ($right_user['delete_news'] AND isset($_GET['v2']) AND $_GET['v2'] == "delete" AND isset($_GET['v3']) AND !empty($_GET['v3']) AND (!isset($included) OR $included == 0)) {
	...
	if (sql_query($sql_sup) != false) {
		$page['L_message_news'] = $lang['news']['form_news_sup_1'];
	} else {
		$page['L_message_news'] = $lang['news']['form_news_sup_0'];
	}
	sql_close($sgbd);
}
...
if (!isset($var['condition']) OR $var['condition'] == "") {
	...
	if (!$right_user['edit_news']) {
		array_push($condition, " news_release  <= NOW() ");
	}

	/* admin */
	if (!$right_user['edit_news']) {
		array_push($condition, " news_status='2' ");
	}
	...
	if ($nb_condition == 0) {
		$var['condition'] = "";
	} elseif ($nb_condition == "1") {
		$var['condition'] = "WHERE " . $condition['0'];
	} else {
		$var['condition'] = "WHERE " . implode(" AND ", $condition);
	}
}
/**********************/
/* END OF CONDITIONS    */
...
if (!isset($var['limit'])) {
	...
	if (!isset($_GET['v2']) OR $_GET['v2'] == "" OR !preg_match("/page/", $_GET['v2'])) {
		$page_num = "1";
	} else {
		$page_num = explode("_", $_GET['v2']);
		$page_num = $page_num['1'];
	}
...
if (!$right_user['news_list']) {
	...
	while ($ligne = sql_fetch_array($res_news)) {
		...
		if (convert_date($ligne['news_release'], $lang['news']['format_time_form']) == '00:00') {
			$page['news'][$i]['release_time'] = '';
		}
