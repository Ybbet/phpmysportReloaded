...
if (!$right_user['add_file'] AND (!isset($_GET['folder']) OR !preg_match("," . AVATAR_FOLDER . ",", $_GET['folder']))) {
...
if (isset($_GET['mode']) AND $_GET['mode'] == "delete" AND $right_user['delete_file']) {
	...
	if ($delete_rep) {
		$page['L_message_file'] = $lang['file']['form_file_sup_1'];
	} else {
		$page['L_message_file'] = $lang['file']['form_file_sup_0'];
	}
}
...
if (isset($_POST['action']) AND $_POST['action'] == "create_folder" AND $right_user['add_folder']) {
	# we check datas
	if (isset($_POST['new_folder']) AND !empty($_POST['new_folder']) AND !check_file_name($_POST['new_folder'])) {
		$page['erreur'][$nb_erreur]['message'] = $lang['file']['E_invalid_folder_name'];
		$nb_erreur++;
	}

	if (file_exists(ROOT . "/" . FILE_FOLDER . "/" . $_POST['upload_folder'] . $_POST['new_folder'])) {
		$page['erreur'][$nb_erreur]['message'] = $lang['file']['E_exist_folder'];
		$nb_erreur++;
	}
	...
	if ($nb_erreur == 0 AND isset($_POST['new_folder']) AND !empty($_POST['new_folder'])) {
		if (@mkdir(ROOT . "/" . FILE_FOLDER . "/" . $_POST['upload_folder'] . $_POST['new_folder'], 0700)) {
			...
			$page['erreur'][$nb_erreur]['message'] = $lang['file']['E_creation_folder'];
			$nb_erreur++;
		}
	} else {
...
if (isset($_POST['action']) AND $_POST['action'] == "upload_file" AND isset($_FILES['file']) AND $right_user['add_file']) {
	...
	if ($nb_erreur == 0) {
		...
		if ($copy_file) {
			...
			$page['L_message_file'] = $lang['file']['form_file_add_0'];
		}
	} else {
...
if ($page['parent_folder'] == "") {
	$page['parent_folder'] = "/";
}
if ($page['current_folder'] == "") {
	$page['current_folder'] = "/";
}
...
foreach ($tab_file as $f) {
	...
	if (in_array($type, $type_allowed)) {
		...
		if (in_array($type, $type_allowed_video)) {
			$page['file'][$j]['category'] = "video";
		}
		if (in_array($type, $type_allowed_text)) {
			$page['file'][$j]['category'] = "text";
		}
		if (in_array($type, $type_allowed_flash)) {
			$page['file'][$j]['category'] = "flash";
		}
		if (in_array($type, $type_allowed_application)) {
			$page['file'][$j]['category'] = "application";
		}

		switch ($type) {
			case "jpg" :
				if (function_exists('imagecreatefromjpeg')) {
					$img = @imagecreatefromjpeg($rep . $f);
				}
				break;
			case "png" :
				if (function_exists('imagecreatefrompng')) {
					$img = @imagecreatefrompng($rep . $f);
				}
				break;
			case "gif" :
				if (function_exists('imagecreatefromgif')) {
					$img = @imagecreatefromgif($rep . $f);
				}
				break;
			default :
				$img = false;
