...
if (isset($_POST) AND !empty($_POST) AND $right_user['configuration']) {
	if (!isset($_POST['title']) OR $_POST['title'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_empty_title'];
		$nb_erreur++;
	}

	if (!isset($_POST['url']) OR $_POST['url'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_empty_url'];
		$nb_erreur++;
	}
	...
	*/
	if (!isset($_POST['root']) OR $_POST['root'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_empty_root'];
		$nb_erreur++;
	} elseif (!file_exists($_POST['root'] . "/index.php")) {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_invalid_root'];
		$nb_erreur++;
	}

	if (isset($_POST['email']) AND !empty($_POST['email']) AND !check_email($_POST['email'])) {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_invalid_email'];
		$nb_erreur++;
	}

	if (isset($_POST['mail']) AND $_POST['mail'] == 1 AND preg_match('mail', ini_get('disable_functions'))) {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_disable_mail'];
		$nb_erreur++;
	} elseif (isset($_POST['mail']) AND $_POST['mail'] == 0 AND isset($_POST['registration_mail']) AND $_POST['registration_mail'] == 1) {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_invalid_registration_mail'];
		$nb_erreur++;
	}

	if (isset($_POST['mail']) AND $_POST['mail'] == 1 AND (empty($_POST['email']) OR empty($_POST['sender_name']))) {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_invalid_sender_name'];
		$nb_erreur++;
	}

	if (!isset($_POST['host']) OR $_POST['host'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_empty_host'];
		$nb_erreur++;
	}
	if (!isset($_POST['user_base']) OR $_POST['user_base'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_empty_user_base'];
		$nb_erreur++;
	}
	if (!isset($_POST['name_base']) OR $_POST['name_base'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_empty_name_base'];
		$nb_erreur++;
	}

	# we try to connect to the database server and to select the base
	if (!@mysqli_connect($_POST['host'], $_POST['user_base'], $_POST['pass_base'])) {
		$page['erreur'][$nb_erreur]['message'] = $lang['administration']['E_invalid_connection_base'];
		$nb_erreur++;
	} elseif (!mysqli_select_db($_POST['name_base'])) {
	...
	if ($nb_erreur == 0) {
		...
		if (!isset($_POST['pass_base']) OR empty($_POST['pass_base'])) {
			$_POST['pass_base'] = SGBD_PWD;
		}
		...
		if (isset($_POST['root'])) {
			$page['value_root'] = $_POST['root'];
		}
		if (isset($_POST['email'])) {
			$page['value_email'] = $_POST['email'];
		}
		if (isset($_POST['sender_name'])) {
			$page['value_sender_name'] = $_POST['sender_name'];
		}

		if (isset($_POST['host'])) {
			$page['value_host'] = $_POST['host'];
		}
		if (isset($_POST['user_base'])) {
			$page['value_user_base'] = $_POST['user_base'];
		}
		if (isset($_POST['name_base'])) {
			$page['value_name_base'] = $_POST['name_base'];
		}
		if (isset($_POST['pass_base'])) {
			$page['value_pass_base'] = $_POST['pass_base'];
		}
		if (isset($_POST['prefix'])) {
			$page['value_prefix'] = $_POST['prefix'];
		}
		if (isset($_POST['url_rewrite'])) {
			$page['value_url_rewrite'] = $_POST['url_rewrite'];
		}
		if (isset($_POST['website_status'])) {
			$page['value_website_status'] = $_POST['website_status'];
		}
		if (isset($_POST['template'])) {
			$page['value_template'] = $_POST['template'];
		}
		if (isset($_POST['avatar_folder'])) {
			$page['value_avatar_folder'] = $_POST['avatar_folder'];
		}
		if (isset($_POST['language'])) {
			$page['value_language'] = $_POST['language'];
		}
		if (isset($_POST['mail'])) {
			$page['value_mail'] = $_POST['mail'];
		}

		if (isset($_POST['nb_news'])) {
			$page['value_nb_news'] = $_POST['nb_news'];
		}
		if (isset($_POST['nb_club'])) {
			$page['value_nb_club'] = $_POST['nb_club'];
		}
		if (isset($_POST['nb_team'])) {
			$page['value_nb_team'] = $_POST['nb_team'];
		}
		if (isset($_POST['nb_member'])) {
			$page['value_nb_member'] = $_POST['nb_member'];
		}
		if (isset($_POST['nb_match'])) {
			$page['value_nb_match'] = $_POST['nb_match'];
		}
		if (isset($_POST['nb_competition'])) {
			$page['value_nb_competition'] = $_POST['nb_competition'];
		}
		if (isset($_POST['nb_field'])) {
			$page['value_nb_field'] = $_POST['nb_field'];
		}
		if (isset($_POST['nb_forum_topic'])) {
			$page['value_nb_forum_topic'] = $_POST['nb_forum_topic'];
		}
		if (isset($_POST['nb_forum_message'])) {
			$page['value_nb_forum_message'] = $_POST['nb_forum_message'];
		}
		if (isset($_POST['home_nb_news'])) {
			$page['value_home_nb_news'] = $_POST['home_nb_news'];
		}
		if (isset($_POST['home_nb_match'])) {
			$page['value_home_nb_match'] = $_POST['home_nb_match'];
		}

		if (isset($_POST['ms_show'])) {
			$page['value_ms_show'] = $_POST['ms_show'];
		}
		if (isset($_POST['ms_column'])) {
			$page['value_ms_column'] = $_POST['ms_column'];
		}
		if (isset($_POST['ms_nb_club_max'])) {
			$page['value_ms_nb_club_max'] = $_POST['ms_nb_club_max'];
		}
		if (isset($_POST['ms_season'])) {
			$page['value_ms_season'] = $_POST['ms_season'];
		}
		if (isset($_POST['ms_competition'])) {
			$page['value_ms_competition'] = $_POST['ms_competition'];
		}
		if (isset($_POST['ms_round'])) {
			$page['value_ms_round'] = $_POST['ms_round'];
		}
		if (isset($_POST['ms_show_form'])) {
			$page['value_ms_show_form'] = $_POST['ms_show_form'];
		}

		if (isset($_POST['registration'])) {
			$page['value_registration'] = $_POST['registration'];
		}
		if (isset($_POST['registration_mail'])) {
			$page['value_registration_mail'] = $_POST['registration_mail'];
		}
	}
}
...
if (isset($page['value_url_rewrite']) AND $page['value_url_rewrite'] == 0) {
	$page['checked_url_rewrite_no'] = "checked=\"checked\"";
} else {
	$page['checked_url_rewrite_yes'] = "checked=\"checked\"";
}
...
if (isset($page['value_website_status']) AND $page['value_website_status'] == 1) {
	$page['checked_site_open'] = "checked=\"checked\"";
} else {
	$page['checked_site_closed'] = "checked=\"checked\"";
}
...
if ($page['show_form']) {
	...
	foreach ($ms_show AS $value) {
		...
		if ($page['value_ms_show'] == $value) {
			$page['ms_show'][$i]['checked'] = 'checked="checked"';
		} else {
			$page['ms_show'][$i]['checked'] = '';
		}
		$i++;
	}

	if ($page['value_ms_show_form'] == 1) {
		$page['checked_ms_show_form'] = 'checked="checked"';
	}
	...
	unset($included);
	if ($page['show_round'] == 1) {
		$page['display_round'] = "block";
	}
