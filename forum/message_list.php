	...
	if ($nb_erreur == 0) {
		...
		if ($spam == 1) {
			...
			if (sql_query($sql_add) != false) {
				...
				$page['L_message_add'] = $lang['forum']['form_message_add_0'];
			}
		...
		} else //if($right_user['edit_message'])
		{
			...
			$sgbd = sql_connect();
			if (sql_query($sql_edit) != false) {
				$page['L_message_add'] = $lang['forum']['form_message_edit_1'];
			} else {
				$page['L_message_add'] = $lang['forum']['form_message_edit_0'];
			}
			sql_close($sgbd);

			if ($_POST['topic'] != "0") {
				$url_redirection = convert_url("index.php?r=" . $lang['general']['idurl_forum'] . "&v1=" . $forum['forum_idurl'] . "&v2=topic_" . $_POST['topic']);
			} else {
				$url_redirection = convert_url("index.php?r=" . $lang['general']['idurl_forum'] . "&v1=" . $forum['forum_idurl'] . "&v2=topic_" . $_POST['id']);
			}
...
if ($right_user['delete_message'] AND isset($_GET['v4']) AND isset($_GET['v3']) AND $_GET['v3'] == "delete") {
	...
	if ($nb_ligne != "0") {
		...
		$sgbd = sql_connect();
		if (sql_query($sql_sup)) {
			$page['L_message_message'] = $lang['forum']['form_message_sup_1'];
		} else {
			$page['L_message_message'] = $lang['forum']['form_message_sup_0'];
		}
...
if (!isset($var['limit'])) {
	...
	/* PAGINATION */
	/**************/
	if (!isset($_GET['v3']) OR $_GET['v3'] == "" OR !preg_match("/page/", $_GET['v3'])) {
		$page_num = "1";
	} else {
		$page_num = explode("_", $_GET['v3']);
		$page_num = $page_num['1'];
	}
...
if ($nb_ligne == "0") {
	...
	while ($ligne = sql_fetch_array($res_message)) {
		if ($i == "0") {
			$page['title'] = $ligne['message_title'];
			if (empty($page['value_title'])) {
				$page['value_title'] = $page['title'];
			}
		}
		...
		if ($right_user['delete_message']) {
			$page['message'][$i]['ip'] = $ligne['member_ip'];
		}
		...
		if ($ligne['member_avatar'] != null) {
			$page['message'][$i]['avatar'] = $ligne['member_avatar'];
		}
		$page['message'][$i]['L_guest'] = "";

		if ($ligne['member_login'] == null) {
			$page['message'][$i]['L_guest'] = $lang['forum']['guest'];
		}
