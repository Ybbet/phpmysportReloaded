...
if (!@include_once(create_path("include/lg_general_" . LANG . ".php"))) {
	echo "ERREUR : Language not supported/Langue non support�e";
	exit();
}
...
if (isset($_SESSION['session_member_id'])) {
	define("MEMBER", "1", 1);
	define("MEMBER_ID", $_SESSION['session_member_id'], 1);
} else {
	define("MEMBER", "0", 1);
	define("MEMBER_ID", "0", 1);
}
/* END SESSIONS */
...
if ($affichage == 1) {
	...
	if (isset($_GET['r']) AND $_GET['r'] != "") {
		...
		switch ($_GET['r']) {
			case $lang['general']['idurl_admin'] :
				require_once(create_path("administration/include.php"));
				$index['class_home'] = 'on';
				break;
			case $lang['general']['idurl_news'] :
				require_once(create_path("news/include.php"));
				$index['class_news'] = 'on';
				break;
			case $lang['general']['idurl_information'] :
				require_once(create_path("information/include.php"));
				$index['class_information'] = 'on';
				break;
			case $lang['general']['idurl_club'] :
				require_once(create_path("club/include.php"));
				$index['class_club'] = 'on';
				break;
			case $lang['general']['idurl_match'] :
				require_once(create_path("match/include.php"));
				$index['class_match'] = 'on';
				break;
			case $lang['general']['idurl_member'] :
				require_once(create_path("member/include.php"));
				$index['class_member'] = 'on';
				break;
			case $lang['general']['idurl_file'] :
				require_once(create_path("file/include.php"));
				$index['class_file'] = '';
				break;
			case $lang['general']['idurl_team'] :
				require_once(create_path("team/include.php"));
				$index['class_team'] = 'on';
				break;
			case $lang['general']['idurl_forum'] :
				require_once(create_path("forum/include.php"));
				$index['class_forum'] = 'on';
				break;
			case $lang['general']['idurl_competition'] :
				require_once(create_path("competition/include.php"));
				$index['class_competition'] = 'on';
				break;
			case $lang['general']['idurl_field'] :
				require_once(create_path("field/include.php"));
				$index['class_field'] = 'on';
				break;
			case $lang['general']['idurl_forum'] :
				require_once(create_path("forum/include.php"));
				$index['class_forum'] = 'on';
...
/* title */
if (isset($page['meta_title']) AND $page['meta_title'] != "") {
	$index['meta_title'] = SITE_TITLE . " &gt; " . $page['meta_title'];
} else {
	$index['meta_title'] = SITE_TITLE;
}
...
if ($in_plugin == 1) {
	...
	$index['contenu'] = parse_template(TPL_URL . $page['template'], $page);
}
/**********************/
...
*/
if (preg_match("/fen_pop/i", $_SERVER['QUERY_STRING'])) {
	$page['fen'] = "pop";
}
if (preg_match("/fen_impression/i", $_SERVER['QUERY_STRING'])) {
	$page['fen'] = "impression";
}
