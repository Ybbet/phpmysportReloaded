...
if (isset($_SESSION['mode'])) {
	$page['value_mode'] = $_SESSION['mode'];
}
if (isset($_SESSION['club_name'])) {
	$page['value_club_name'] = $_SESSION['club_name'];
}
if (isset($_SESSION['sport'])) {
	$page['value_sport'] = $_SESSION['sport'];
}

if (isset($_POST) AND !empty($_POST)) {
	if (!isset($_POST['mode']) OR $_POST['mode'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_mode'];
		$nb_erreur++;
	} elseif ($_POST['mode'] == "club" AND (!isset($_POST['club_name']) OR empty($_POST['club_name']))) {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_club_name'];
		$nb_erreur++;
	}

	if (!isset($_POST['sport']) OR $_POST['sport'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_sport'];
		$nb_erreur++;
	}
	...
	if ($nb_erreur == 0) {
		...
		if (isset($_POST['mode'])) {
			$page['value_mode'] = $_POST['mode'];
		}
		if (isset($_POST['club_name'])) {
			$page['value_club_name'] = $_POST['club_name'];
		}
		if (isset($_POST['sport'])) {
			$page['value_sport'] = $_POST['sport'];
		}
	}
}
...
switch ($page['value_mode']) {
	case "club" :
		$page['checked_club'] = "checked=\"checked\"";
		$page['style_club_name'] = "block";
		break;
	case "comite" :
		$page['checked_comite'] = "checked=\"checked\"";
		break;
}
...
for ($i = 0; $i < $nb_sport; $i++) {
	...
	if ($page['value_sport'] == $sport_supporte[$i]) {
		$page['sport'][$i]['checked'] = "checked=\"checked\"";
	} else {
		$page['sport'][$i]['checked'] = "";
	}
}
