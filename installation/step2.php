...
if (isset($_SESSION['title'])) {
	$page['value_title'] = $_SESSION['title'];
}
if (isset($_SESSION['url'])) {
	$page['value_url'] = $_SESSION['url'];
}
if (isset($_SESSION['root'])) {
	$page['value_root'] = $_SESSION['root'];
}
if (isset($_SESSION['email'])) {
	$page['value_email'] = $_SESSION['email'];
}
if (isset($_SESSION['host'])) {
	$page['value_host'] = $_SESSION['host'];
}
if (isset($_SESSION['user_base'])) {
	$page['value_user_base'] = $_SESSION['user_base'];
}
if (isset($_SESSION['name_base'])) {
	$page['value_name_base'] = $_SESSION['name_base'];
}
if (isset($_SESSION['prefix'])) {
	$page['value_prefix'] = $_SESSION['prefix'];
}
if (isset($_SESSION['user_admin'])) {
	$page['value_user_admin'] = $_SESSION['user_admin'];
}
if (isset($_SESSION['name_admin'])) {
	$page['value_name_admin'] = $_SESSION['name_admin'];
}
if (isset($_SESSION['firstname_admin'])) {
	$page['value_firstname_admin'] = $_SESSION['firstname_admin'];
}
if (isset($_SESSION['url_rewrite'])) {
	$page['value_url_rewrite'] = $_SESSION['url_rewrite'];
}
...
if (isset($_POST) AND !empty($_POST)) {
	if (!isset($_POST['title']) OR $_POST['title'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_title'];
		$nb_erreur++;
	}
	if (!isset($_POST['url']) OR $_POST['url'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_url'];
		$nb_erreur++;
	}
// elseif(!@fopen($_POST['url'],"r")) { $page['erreur'][$nb_erreur]['message']=$lang['installation']['E_invalid_url']; $nb_erreur++; }
	...
	if (!isset($_POST['root']) OR $_POST['root'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_root'];
		$nb_erreur++;
	}
	...
	if (isset($_POST['email']) AND !empty($_POST['email']) AND !check_email($_POST['email'])) {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_invalid_email'];
		$nb_erreur++;
	}

	if (!isset($_POST['host']) OR $_POST['host'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_host'];
		$nb_erreur++;
	}
	if (!isset($_POST['user_base']) OR $_POST['user_base'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_user_base'];
		$nb_erreur++;
	}
	if (!isset($_POST['name_base']) OR $_POST['name_base'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_name_base'];
		$nb_erreur++;
	}
	if (!isset($_POST['user_admin']) OR $_POST['user_admin'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_user_admin'];
		$nb_erreur++;
	}

	if (!isset($_POST['pass_admin']) OR $_POST['pass_admin'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_pass_admin'];
		$nb_erreur++;
	}
	if (!isset($_POST['pass_2_admin']) OR $_POST['pass_2_admin'] == "") {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_empty_pass_2_admin'];
		$nb_erreur++;
	}
	if ($_POST['pass_admin'] != $_POST['pass_2_admin']) {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_invalid_pass_admin'];
		$nb_erreur++;
	}

	# on essaie de se connecter au serveur
	if (!@mysqli_connect($_POST['host'], $_POST['user_base'], $_POST['pass_base'])) {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_invalid_connection_base'];
		$nb_erreur++;
	}
	...
	if ($nb_erreur == 0) {
		...
		if (isset($_POST['prefix'])) {
			$page['value_name_base'] = $_POST['prefix'];
		}
		if (isset($_POST['user_admin'])) {
			$page['value_user_admin'] = $_POST['user_admin'];
		}
		if (isset($_POST['name_admin'])) {
			$page['value_name_admin'] = $_POST['name_admin'];
		}
		if (isset($_POST['firstname_admin'])) {
			$page['value_firstname_admin'] = $_POST['firstname_admin'];
		}
