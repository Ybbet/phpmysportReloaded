...
$db = mysqli_connect($_SESSION['host'], $_SESSION['user_base'], $_SESSION['pass_base']);
if (!mysqli_select_db($_SESSION['name_base'], $db)) {
	if (isset($_POST['create_base']) AND $_POST['create_base'] == 1) {
		$req_creation = "CREATE DATABASE `" . $_SESSION['name_base'] . "` ";
		if (mysqli_query($db, $req_creation)) {
			$page['message'][$nb_message]['message'] = $lang['installation']['creation_base_ok'];
			$nb_message++;
			$_SESSION['creation_base'] = 1;
		} else {
			$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_creation_base'];
			$nb_erreur++;
...
if (isset($_SESSION['creation_base']) AND $_SESSION['creation_base'] == 1 AND (!isset($_SESSION['table']) OR $_SESSION['table'] != 1)) {
	...
	$res_table = mysqli_query($db, $req_table);
	if ($res_table != false) {
		$nb_table = mysqli_num_rows($res_table);
	} else {
		$nb_table = 0;
	}
	...
	if ($nb_table != 0) {
		if (isset($_POST['delete_table']) AND $_POST['delete_table'] == "1") {
			...
			while ($ligne_table = mysqli_fetch_array($res_table)) {
				$liste_table[] = "`" . $ligne_table['0'] . "`";
			}
			$req_delete_table = "DROP TABLE  " . implode(", ", $liste_table) . " ";

			if (mysqli_query($db, $req_delete_table)) {
				$page['message'][$nb_message]['message'] = $lang['installation']['suppression_table_ok'];
				$nb_message++;
				$_SESSION['table'] = 1;
			} else {
				$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_suppression_table'] . " " . mysqli_error($db) . " ";
				$nb_erreur++;
			}
...
if (isset($_SESSION['table']) AND $_SESSION['table'] == 1 AND (!isset($_SESSION['creation_table']) OR $_SESSION['creation_table'] != 1)) {
	...
	if ($fd = fopen($filename, "r")) {
		...
		$nb_table = 0;
		$db = mysqli_connect($_SESSION['host'], $_SESSION['user_base'], $_SESSION['pass_base']);
		mysqli_select_db($_SESSION['name_base'], $db);
		while (!feof($fd)) {
			$ligne = fgets($fd, 4096);
			...
			if (preg_match("/;/i", $ligne)) {
				...
				if (mysqli_query($db, $req_creation_table)) {
					$var['nb_table_ok']++;
				} else {
					$var['nb_table_pbm']++;
				}
		...
		if ($var['nb_table_ok'] != 0) {
			$page['message'][$nb_message]['message'] = text_replace($lang['installation']['creation_table_ok'], $var);
			$nb_message++;
		}
		if ($var['nb_table_pbm'] != 0) {
			$page['erreur'][$nb_erreur]['message'] = text_replace($lang['installation']['E_creation_table'], $var);
			$nb_erreur++;
			$page['value_erreur'] = "creation_table";
		}

		if ($nb_table != 0 AND $var['nb_table_ok'] == $nb_table) {
			$_SESSION['creation_table'] = 1;
		}
	}
}

if (isset($_SESSION['creation_table']) AND $_SESSION['creation_table'] == 1) {
	$page['message'][$nb_message]['message'] = $lang['installation']['creation_tables_ok'];
	$nb_message++;
}
...
if (isset($_SESSION['creation_table']) AND $_SESSION['creation_table'] == 1 AND (!isset($_SESSION['creation_user']) OR $_SESSION['creation_user'] != 1)) {
	...
	if (mysqli_query($db, $req_creation_use)) {
		$_SESSION['creation_user'] = 1;
	} else {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_creation_user'] . " " . mysqli_error($db) . " ";
		$nb_erreur++;
...
if (isset($_SESSION['creation_user']) AND $_SESSION['creation_user'] == 1) {
	$page['message'][$nb_message]['message'] = $lang['installation']['creation_user_ok'];
	$nb_message++;
}
...
if ($_SESSION['mode'] != "club") {
	...
	if (mysqli_query($db, $req_creation_club)) {
		$_SESSION['creation_club'] = 1;
		$_SESSION['club'] = "1";
	} else {
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_creation_club'] . " " . mysqli_error($db) . " ";
		$nb_erreur++;
...
if (isset($_SESSION['creation_club']) AND $_SESSION['creation_club'] == 1) {
	$page['message'][$nb_message]['message'] = $lang['installation']['creation_club_ok'];
	$nb_message++;
}
...
if (isset($_SESSION['creation_user']) AND $_SESSION['creation_user'] == 1 AND (!isset($_SESSION['creation_conf']) OR $_SESSION['creation_conf'] != 1)) {
	...
	if ($fd = @fopen($fichier_conf_site, "w")) {
		// cr�ation du fichier de connection
...
if (isset($_SESSION['creation_conf']) AND $_SESSION['creation_conf'] == 1 AND (!isset($_SESSION['insertion_data']) OR $_SESSION['insertion_data'] != 1)) {
	...
	for ($i = 0; $i < $nb_req_insertion_data - 1; $i++) {
		...
		if (mysqli_query($db, $req_insertion_data[$i])) {
			$nb_req_ok++;
		} else {
			$nb_req_pbm++;
		}
	...
	if ($nb_req_ok == $nb_req_insertion_data - 1) {
		...
		$page['erreur'][$nb_erreur]['message'] = $lang['installation']['E_insertion_data'];
		$nb_erreur++;
...
if (isset($_SESSION['insertion_data']) AND $_SESSION['insertion_data'] == 1) {
	$page['message'][$nb_message]['message'] = $lang['installation']['insertion_data_ok'];
	$nb_message++;
}
...
if (isset($_SESSION['creation_htaccess']) AND $_SESSION['creation_htaccess'] == 1) {
	$page['message'][$nb_message]['message'] = $lang['installation']['creation_htaccess_ok'];
	$nb_message++;
}
