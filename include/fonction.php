...
function convert_url($s, $c = URL_REWRITE) {
	if (isset($_GET['fen']) AND $_GET['fen'] == "pop") {
		$s .= "&fen=pop";
	}
	...
	if (($c == 1) OR (URL_REWRITE == 1 AND $c == 2)) {
		...
		if ($dernier_car != "/" AND $dernier_car != "") {
			$s .= ".html";
		}
	}
...
function convert_date($date, $format) {
	if ($date == '0000-00-00 00:00:00' OR $date == '0000-00-00' OR $date == '' OR $date == null) {
		...
		if (phpversion() < 5.0 AND $year < 1970) {
			...
			$new_format = preg_replace(',%a|%A|%u,', '', $format); # we remove days information from the format because they would be wrong
			$new_date = strftime($new_format, strtotime($new_date)); # we convert the date
			$new_date = preg_replace(',1980,', $year, $new_date); # we put back the real year
...
function calcul_age($dob) {
	...
	if ($month_day < $month_nais) {
		$age = $age--;
	} elseif ($day_day < $day_nais && $month_day == $month_nais) {
		$age = $age--;
	}
		...
		*/
		if ($nb_page > 10) {
			if ($page_num <= 5) {
				$start = 1;
				$end = 10;
			} # ici on affiche les 10 premiers number
			elseif ($page_num > $nb_page - 5) {
				$start = $nb_page - 9;
				$end = $nb_page;
			} # on affiche les 10 derniers numbers
			else {
				$start = $page_num - 5;
				$end = ($page_num + 4) > $nb_page ? $nb_page : $page_num + 4;
			} # on affiche les 10 numbers centres sur le number of the current page
...
function html2txt($text) {
	if ($text != "") {
		...
		$text = preg_replace(",[[:space:]]+,", " ", trim($text));
...
*/
// si $b est � 1, on force la coupe du mot
function text_tronquer($text, $nb, $b = 0) {
	// Test si la longueur du text d�passe la limite
	if (strlen($text) > $nb) {
		$text = substr($text, 0, $nb);
		// R�écupération de la position du dernier espace (aend d'éviter de tronquer un mot)
		if ($b) {
			$text = substr($text, 0, $nb);
		} else {
...
function filesize_format($bytes, $decimal = '.', $spacer = ' ', $lowercase = false) {
	...
	$units = array(
		'YB' => 1208925819614629174706176, // yottabyte
